...
function removeFirstOccurrences(str, value) {
  const red = str.indexOf(value);
  if (red < 0) return str;
  return str.substring(0, red) + str.substring(red + value.length, str.length);
}
...
function removeLastOccurrences(str, value) {
  const red = str.lastIndexOf(value);
  if (red < 0) return str;
  return str.substring(0, red) + str.substring(red + value.length, str.length);
}
...
function reverseString(str) {
  ...
  for (let index = str.length - 1; index >= 0; index -= 1) {
    result = result.concat(str.at(index));
  }
  return result;
}
